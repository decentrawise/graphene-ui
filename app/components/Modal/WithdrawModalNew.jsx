...
import {Modal, Button, Select, Input} from "graphene-ui-style-guide";
...
import {ChainStore} from "@decentrawise/graphenejs";
import {getAssetAndGateway, getIntermediateAccount} from "common/gatewayUtils";
import chainConfig from "chain/chainConfig";
...
class WithdrawModalNew extends React.Component {
    ...
    _getAssetPairVariables(props = this.props, state = this.state) {
        ...
        let isCORE = false;
        if (coreAsset) {
            if (selectedAsset == coreAsset.get("symbol")) isCORE = true;
        } else if (selectedAsset == chainConfig.core_asset) {
            isCORE = true;
        }
    ...
    _getAvailableAssets(state = this.state) {
        let coreAccount = this.props.account;
        let fee_asset_types = [];
        if (!(coreAccount && coreAccount.get("balances"))) {
            return {fee_asset_types};
        }
        let account_balances = coreAccount.get("balances").toJS();
    ...
    onAssetChanged(value) {
        ...
        if (value == chainConfig.core_asset) {
            stateObj = {isCORE: true};
        }
    ...
    onCOREAccountNameChanged(coreAccountName) {
        if (!coreAccountName) this.setState({coreAccount: null});
        this.setState({coreAccountName, coreAccountError: null});
    }

    onCOREAccountChanged(coreAccount) {
        this.setState({coreAccount, coreAccountError: null});
    }
    ...
    render() {
        ...
        const shouldDisable = isCORE
            ? !quantity || !coreAccount
            : !assetAndGateway ||
              !quantity ||
              !address ||
              !canCoverWithdrawal ||
            ...
            >
                <div className="grid-block vertical no-overflow">
                    <div className="modal__body" style={{paddingTop: 0}}>
                        ...
                        {!isCORE && selectedAsset && !selectedGateway ? (
                        ...
                        {/*ESTIMATED VALUE*/}
                        {/*
                (assetAndGateway || quantity) && !isCORE ?
                <div>
            ...
            */}
                        ...
                        {isCORE ? (
                            <div style={{marginBottom: "1em"}}>
                                <AccountSelector
                                    label="transfer.to"
                                    accountName={state.coreAccountName}
                                    onChange={this.onCOREAccountNameChanged.bind(
                                        this
                                    )}
                                    onAccountChanged={this.onCOREAccountChanged.bind(
                                        this
                                    )}
                                    account={state.coreAccountName}
                                    size={60}
                                    error={state.coreAccountError}
                                />
                            </div>
                        ) : null}
