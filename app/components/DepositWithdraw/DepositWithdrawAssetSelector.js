...
import {Select} from "graphene-ui-style-guide";
...
class DepositWithdrawAssetSelector extends React.Component {
    ...
    getAssets() {
        let {backedCoins, include, includeCORE} = this.props;
        ...
        backedCoins.forEach(coin => {
            assets = assets
                ...
                .filter(item => {
                    if (item.id == chainConfig.core_asset) {
                        return true;
                    }
                    if (include) {
        ...
        if (!(includeCORE === false)) {
            assets.push({
                id: chainConfig.core_asset,
                label: chainConfig.core_asset,
                gateway: ""
            });
        }
