...
import {Tooltip, Icon as AntIcon} from "graphene-ui-style-guide";
...
class AccountPortfolioList extends React.Component {
    ...
    _renderBuy = (symbol, canBuy, assetName, emptyCell, balance) => {
        if (symbol === chainConfig.core_asset && balance <= 1000000) {
    ...
    getHeader(atLeastOneHas) {
        ...
        let headerItems = [
            ...
            {
                className: "column-hide-medium",
                title: atLeastOneHas.depositOnlyCORE ? (
                    <React.Fragment>
                        <Tooltip
    ...
    _renderBalances(balanceList, optionalAssets, visible) {
        ...
        (balanceList || []).forEach(balance => {
            ...
            const canDeposit =
                ...
                asset.get("symbol") == chainConfig.core_asset;
    ...
    render() {
        ...
        balanceRows.forEach(_item => {
            ...
            if (!!_item.deposit && _item.deposit !== "-") {
                if (
                    _item.key == chainConfig.core_asset &&
                    GatewayStore.anyAllowed()
                ) {
                    atLeastOneHas.depositOnlyCORE =
                        _item.key == chainConfig.core_asset &&
                        !atLeastOneHas.deposit;
