...
In order to increase your exposure to CORE and offer liquidity to BitAssets, such
...
A Graphene network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 CORE as it is traded on external exchanges.
...
A Graphene network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/CORE).
...
market rates up to the SQP and close the position. The remaining CORE of the
...
increase, an additional amount of CORE is locked as collateral, while reducing
...
particular bitAsset to hand it over to the Graphene network. After that, the
